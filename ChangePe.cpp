...
char OUT newSectionOutFilePathForExe[] = "C:/Users/Admin/Desktop/PETEST/移动导入表加了一个节.exe";
char OUT newSectionOutFilePathForDLL[] = "C:/Users/Admin/Desktop/PETEST/移动导入表加了一个节.DLL";
...
DWORD SectionInject(LPVOID pFileBuffer, LPSTR inFilePath, LPSTR outFilePath, DWORD sectionNO, BYTE * shellCode)
{
	...
	//printf("%d", sizeof(shellCode));//sizeof 计算四字节 结果为4
	injectAddress = pInjectSectionHeader->SizeOfRawData + pInjectSectionHeader->PointerToRawData - (sizeof(shellCode) * (4 + 1));	//区段结尾
...
DWORD MoveImportDescriptor(LPSTR inFilePath, LPSTR outFilePath, DWORD numOfNewImportDescriptors)
{
	...
	while (pImportDescriptor->OriginalFirstThunk)
	{
		...
		LPDWORD pFirstThunkValue = (DWORD *)((DWORD)pFileBuffer + foaFirstThunk);
		*pFirstThunkValue = 1;	//最后修改即可
		pImportDescriptor++;
	}
	
	//382-386 454-460移动表结构并修复数据目录表
	pImportDescriptor = (PIMAGE_IMPORT_DESCRIPTOR)((DWORD)pImportDescriptor - sizeOfImportDescriptor);
	sizeOfImportDescriptor = sizeOfImportDescriptor + numOfNewImportDescriptors * SIZE_OF_IMPORT_DESCRIPTOR;	//为之后添加导入表打提前量
